#!/usr/bin/env python3
"""
Shell autocompletion setup for PornHub CLI.
...
def get_completion_command() -> str:
    ...
    if shutil.which("pornhub") is None:
        sys.exit(
            "pornhub command not found. "
    ...
    if shell == "zsh":
        return 'eval "$(_PORNHUB_COMPLETE=zsh_source pornhub)"'
    if shell == "bash":
        return 'eval "$(_PORNHUB_COMPLETE=bash_source pornhub)"'
    if shell == "powershell":
        return "& $env:_PORNHUB_COMPLETE=powershell_source pornhub" + " | Out-String | Invoke-Expression"
